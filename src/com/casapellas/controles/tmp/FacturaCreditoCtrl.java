...
public class FacturaCreditoCtrl {
	...
	public List<Credhdr> buscarFacturaCreditoNoLigas(String sParametro,String sMoneda,String sCodComp,
										String sCodunineg, String sCodsuc,F55ca014[] f14, Date fechafactura ){
		List<Credhdr> facturas = null;
		
		try {
			
			String query = " select * from "+PropertiesSystem.ESQUEMA+".Credhdr " +
				"where trim(RPDCTO) = '' and codcli =  " + sParametro +  
				" and fecha = '" + new SimpleDateFormat("yyyy-MM-dd").format ( fechafactura  )+"' ";
			
			if( sMoneda.trim().compareTo("01") != 0)
				query = query + " and moneda  = '"+ sMoneda.trim()+"'";
			if( sCodComp.trim().compareTo("01") != 0)
				query = query + " and trim(codcomp) = '"+ sCodComp.trim()+"'";
			if( sCodsuc.trim().compareTo("01") != 0)
				query = query + " and trim( codsuc ) = '"+ sCodsuc.trim()+"'";
			if( sCodunineg.compareTo("01") != 0)
				query = query + " and trim(codunineg) = '"+ sCodunineg.trim()+"'";
			
			facturas = ConsolidadoDepositosBcoCtrl.executeSqlQuery(query, Credhdr.class, true) ;
			
			if(facturas == null)
				return null;
			
			CuotaCtrl.llenarInfoFactura(facturas, f14);
	...
	public List<Credhdr> buscarFacturasNoLigas(String sParametro,String sMoneda,String sCodComp,
		...
		String sql = "SELECT CAST(TRIM(RPDCT) AS VARCHAR(2) CCSID 37) TIPOFACTURA,RPDOC AS NOFACTURA,CAST (RPSFX AS VARCHAR(3) CCSID 37) AS PARTIDA "+
					...
					" WHERE trim(RPDCTO) = '' AND TRIM(RPPO) = '' AND RPAAP > 0 AND CAST((CAST(DATE(CHAR(1900000 + RPDIVJ)) AS TIMESTAMP)) AS DATE) = '"+fecha+"'";
		...
		try {			
			...
			LogCajaService.CreateLog("buscarFacturasNoLigas", "QRY", sql);
			...
			LogCajaService.CreateLog("buscarFacturasNoLigas", "ERR", ex.getMessage());
		}
 ...
 */
	...
	public List<Credhdr> buscarFacturasCredito(int iParametro,String sMoneda,String sCodComp, 
												int iDesde,int iHasta, ArrayList<Credhdr> lstSelectedFacs,F55ca014[] f14){
		List<Credhdr>lstCredhdr = null;
		Criteria ctr = null;
		Session sesion = null;
		...
		try {
			...
			ctr = sesion.createCriteria(Credhdr.class)
					.add(Restrictions.ne("id.rpdcto", "X9"))
					.addOrder(Order.asc("id.fechavenc"));
			...
			if(lstCredhdr == null || lstCredhdr.isEmpty() ){
				return lstCredhdr = new ArrayList<Credhdr>();
			}
				
			lstCredhdr = CuotaCtrl.llenarInfoFactura(lstCredhdr, f14);
			
			
		} catch (Exception error) {
			LogCajaService.CreateLog("buscarFacturasCredito", "ERR", error.getMessage());
			lstCredhdr = new ArrayList<Credhdr>();			
		}
 ...
 */
	...
	public List<Credhdr> buscarFacturasCredito(int iTipoBusqueda,String sParametro,String sMoneda,
		...
		try {
			...
			ClsParametroCaja obj = new ClsParametroCaja();
			CajaParametro cajaParm = obj.getParametros("22", "0", "DOC_EXCLUIR_BUSQ_CR");
			
			if (cajaParm != null) {
				String[] listaExclusion = cajaParm.getValorAlfanumerico().split(",");
				for (String val : listaExclusion) {
					cr.add(Restrictions.ne("id.tipofactura", val.trim().toUpperCase()));
				}
			}
			...
			try {
				...
				LogCajaService.CreateLog("buscarFacturasCredito", "ERR", e2.getMessage());
			}
		}
...
/***OBTIENE EL DETALLE DE UNA FACTURA DE CREDITO********************************************************************************************************************/
	public List getDetalleFacturaCredito(int iNofactura,String sTipoFactura,String sCodsuc,String sCodunineg){
		...
		String sql = "from Creddet as d where d.id.nofactura = " + iNofactura
				+ " and trim(d.id.tipofactura) = '" + sTipoFactura.trim()
				+ "' " + "and trim(d.id.codsuc) = '" + sCodsuc
				+ "' and trim(d.id.codunineg) = '" + sCodunineg + "'";

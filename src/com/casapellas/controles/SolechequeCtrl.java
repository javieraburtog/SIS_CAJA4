 ...
 */
	public boolean grabarSolecheque(Session sesion, int iCaid,String sCodcomp,String sCodsuc,String sCodunineg,
									...
									String sLogin,String sIdAfiliado,String sFechaPago,BigDecimal bdTasaCambio, int icodcli, int iFechafach){
		...
		try{
			...
			bGrabado = false;
			System.out.println("Se capturo una excepcion en SolechequeCtrl.grabarSolecheque: " + ex);
		}
		return bGrabado;
	}
/**************************************************************************************/
	...
	public boolean  enviarCorreo(String sEncabezado,String sPieCorreo, String sTo, String sFrom,String sNombreFrom, String sCc,String sSubject,
		...
		try {
			...
			MailHelper.SendHtmlEmail(
					...
					new CustomEmailAddress(( sCc!= null && sCc.trim().compareTo("") != 0 )? sCc: sFrom), 
					sSubject, shtml);

		} catch (Exception error) {
			enviado = false;
			System.out.println("Error en SolechequeCtrl.enviarCorreo  " + error);
		}
		return enviado;
	}
...
/**	Permite actualizar una solicitud de cheque a partir de un objeto Vsolecheque	 **/
	public boolean actualizarSolecheque(Session sesion, Vsolecheque v, int iCoduser,String sObservacion,int iCodestado){
		boolean bHecho = true;
		Divisas dv = new Divisas();
		...
		try {
			...
			sc.setFechafac( FechasUtil.DateToJulian(v.getId().getFechadev())); 
			...
			LogCajaService.CreateLog("actualizarSolecheque", "HQRY", LogCajaService.toJson(sc));

			sesion.update(sc);
		} catch (Exception error) {
			bHecho = false;
			LogCajaService.CreateLog("actualizarSolecheque", "HQRY", error.getMessage());
		}
...
/**	Permite actualizar una solicitud de cheque a partir de un objeto Vsolecheque	 **/
	public boolean actualizarSolecheque(Vsolecheque v, int iCoduser,String sObservacion,int iCodestado,Session sesion){
		...
		try {
			...
			bHecho = false;
			LogCajaService.CreateLog("actualizarSolecheque", "ERR", error.getMessage());
		} 
		return bHecho;
	}
/**************************************************************************************/
/**	Permite consultar las solicitudes de cheques a partir de una consulta sql previa **/
	public List obtenerSolicitudes(String sql, int iMaxResult){
		...
		try {
			...
			lstSol = null;
			System.out.println("Error en  SolechequeCtrl.obtenerSolicitudes" + error);
		} finally {
...
/**OBTENER EL NUMERO DE SOLICITUD DE EMISION DE CHEQUE POR COMPANIA, SUCURSAL, Y CAJA*******************************************/
	public Numcaja obtenerNoSolicitud(int iCaid,String sCodcomp,String sCodsuc){
		...
		try{
			...
			System.out.println("Se capturo una excepcion en SolechequeCtrl.obtenerNoSolicitud: " + ex);
		}finally{
			session.close();
		}
		return numcaja;
...
/**ACTUALIZAR EL NUMERO DE SOLICITUD DE EMISION DE CHEQUES******************************************************************************************/
	public boolean actualizarNoSolicitud(Numcaja numcaja){
		...
		try{
			...
			bActualizado = false;
			System.out.println("Se capturo una excepcion en SolechequeCtrl.actualizarNoSolicitud: " + ex);
		}finally{
			session.close();
		}
		return  bActualizado;
